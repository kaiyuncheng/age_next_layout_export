...
export default function CategoryPage({ data }) {
  ...
  const [articlesData, setArticlesData] = useState(categoriesData.article_info.slice(1));
  ...
  return (
    <Layout siteTitle={`幸福熟齡 - ${categoriesData.category_info.name}`}>
      ...
      <BreadCrumb
        titles={[
          {
            title: `${categoriesData.category_info.parent_name}`,
            link: `/categories/${categoriesData.category_info.parent_id}`,
          },
          {
            title: `${categoriesData.category_info.name}`,
            link: `/category/${categoriesData.category_info.category_id}`,
          },
        ]}
      />
      ...
      <div className="sections">
        <div className="max-w-screen-2xl mx-auto px-4 lg:px-2 flex flex-col md:flex-row md:space-x-10 md:space-y-0">
          <div className="relative w-full md:w-3/4 flex flex-col space-y-14 overflow-hidden mb-14">
            <MainArticle topics={categoriesData.article_info[0]} />
            
            <ArticleList topics={articlesData}/>
          </div>
